...
- `tf-engines` の YAML parser は Sigma・Correlation 共有基盤: `src/yaml/` module は Sigma（Phase 5 Sigma 編）と Correlation（Phase 5 Correlation 編 T5-030〜）の両方で利用する。anchor/alias/tag/duplicate key/multi-document marker/block scalar/tab indentation を検出次第 error とする（規範 §14・Schema §7）。
