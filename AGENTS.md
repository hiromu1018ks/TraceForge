...
- `tf-findings` が Phase 6 で追加した依存: `tf-core`（path + version 指定・Finding / Match / Severity / Confidence / AttackMapping / 決定的 ID）・`serde_json`（canonical JSON 構築・dataset STIX bundle の値参照）・`thiserror`（FindingBuildError・AttackDatasetError・UnknownTechniqueError derive・規範 §17.2）。dev-dependency に `tempfile`（テスト用一時 directory・dataset file 読込み検証等）。
- `tf-findings` の module 構成（Phase 6）: `src/lib.rs`（公開 API）・`src/merger.rs`（`FindingBuilder`・`FindingMergeOptions`・`FindingMergeRule`・`FindingMergeSummary`・`MergeGroupId`・`FindingBuildError`・`attach_attack_mappings`・`manual_attack_mapping`・T6-001〜T6-005・規範 §16）・`src/attack/`（`mod.rs`: 公開 API 再エクスポート・`dataset.rs`: `AttackDataset`・`AttackDatasetManifest`・`AttackDatasetError`・STIX bundle からの technique 抽出・T6-006・`technique.rs`: `TechniqueInfo`・`UnknownTechniqueError`・`validate_technique_id_format`・`validate_technique_ids`・T6-007・`mapping.rs`: `from_correlation_rule`・`from_sigma_rule_tags`・`extract_attack_tags_from_sigma`・`built_in_mappings`・`manual_mapping`・T6-008・T6-009）。統合テストは `crates/findings/tests/acceptance_tests.rs`（T6-001〜T6-009 受け入れ条件 20件）。
- `tf-findings` の依存方向: `tf-findings` → `tf-core`（本番依存）。`tf-evidence` / `tf-store` / `tf-parsers` / `tf-engines` へは依存しない（Finding 統合は Match list を入力とし、Evidence snapshot・EventStore・Parser・Engine の内部状態へ影響しないため）。呼出側（Phase 7 の CLI）が3エンジンの Match list と EventStore から Event を集めて `FindingBuilder::build` へ渡す設計。
- `tf-core` の `finding.rs` で Phase 6 に `AttackMapping` 型を拡張した（`tactic` / `source` / `dataset_version` / `dataset_sha256` field を追加）および `AttackMappingSource` enum（`Rule` / `SigmaTag` / `BuiltIn` / `Manual`・規範 §15.3）を新設した。Phase 1 で `attack_mappings: vec![]` を使っているコードは空 list のため後方互換性がある。
- `tf-export` が Phase 7 で追加した依存: `tf-core`（path + version 指定・CaseMetadata・EvidenceItem・ArtifactInstance・Event・Finding・Manifest・canonical JSON・Schema・hash）、`tf-store`（path + version 指定・TimelineKey sort 用）、`serde_json`（canonical JSON 構築・Schema 検証）、`thiserror`（ExportError derive・規範 §17.2）。dev-dependency に `tempfile`（テスト用一時 directory）と `chrono`（テスト用 EventTime 構築）。
- `tf-export` の module 構成（Phase 7）: `src/lib.rs`（公開 API）・`src/case_data.rs`（`CaseData`・`SortedViews`・全 exporter へ共通の入力）・`src/error.rs`（`ExportError`）・`src/sanitize.rs`（`escape_control_chars`・`sanitize_csv_cell`・`html_text_escape`・`html_attribute_escape`・規範 §19・T7-001・T7-004・T7-005）・`src/schema_check.rs`（`check_case_schema_major`・`check_jsonl_schema_major`・互換 §10・T7-009）・`src/manifest.rs`（`ManifestFinalizationInput`・`finalize_manifest`・`manifest_without_run_metadata`・`missing_manifest_fields`・`default_components`・規範 §20・T7-032・T7-033）・`src/text.rs`（Text exporter・規範 §19.1・T7-001）・`src/json.rs`（JSON exporter・Schema §5・T7-002）・`src/jsonl.rs`（JSONL exporter・Schema §6・T7-003）・`src/csv.rs`（CSV exporter・RFC 4180・formula injection 対策・規範 §19.2・T7-004）・`src/html.rs`（HTML exporter・offline・CSP 埋込・規範 §19.3・T7-005・`HTML_CSP` 定数）・`src/timesketch.rs`（Timesketch exporter・TF-TIMESKETCH-1.0・互換 §8・T7-006・`TimesketchSummary`）。統合テストは `crates/export/tests/injection_tests.rs`（T7-008・§21-11）・`crates/export/tests/schema_tests.rs`（T7-009・§21-15）・`crates/export/tests/determinism_tests.rs`（T7-033・規範 §13.1・§20）。
- `tf-cli` が Phase 7 で追加した依存: `tf-core`・`tf-evidence`・`tf-store`・`tf-parsers`・`tf-engines`・`tf-findings`・`tf-export`（何れも path + version 指定・9 command が各 Phase の成果物を統合）、`serde_json`（Case JSON / JSONL 読込）、`sha2`・`hex`（inspect command で file の SHA-256 を算出・規範 §2）、`chrono`（analyze command の run 時刻取得・規範 §13.1）、`tempfile`（EventStore spool file・snapshot 用一時 directory）、`thiserror`（`CliParseError`・`CaseReadError`・`OutputWriteError` derive・規範 §17.2）。外部 CLI crate（clap 等）は使用しない（依存最小化・`--no-hash` を明示的に拒否するため）。
- `tf-cli` の module 構成（Phase 7）: `src/main.rs`（プロセス entry point・stdout/stderr 分離・Exit Code 設定）・`src/lib.rs`（`run(&[String]) -> CliResult` API・テストから直接呼出可能・T7-034）・`src/args.rs`（最小引数 parser・`CliArgs`・`Command`・9種の Args 型・`OutputFormatArg`・`CliParseError`・`--no-hash` 拒否・T7-020・T7-022）・`src/runtime.rs`（`RunContext`・`read_case_from_path`・`parse_case_json`・`parse_jsonl_case`・`write_output`・`CaseReadError`・`OutputWriteError`・T7-027）・`src/version_info.rs`（`TRACEFORGE_VERSION`・`SCHEMA_VERSION_STR`・`COMPATIBILITY_PROFILE`・`BUILD_COMMIT`・`TARGET`・`SIGMA_ENGINE_VERSION`・`CORRELATION_ENGINE_VERSION`・`YARA_X_ENGINE_VERSION`・T7-030）・`src/commands/{analyze,timeline,correlate,sigma,yara,export,rules,inspect,version}.rs`（9 command 実装・製品 §12）。統合テストは `crates/cli/tests/cli_tests.rs`（T7-020〜T7-034 の各 command 挙動 16 test）。
- `tf-export` の依存方向: `tf-export` → `tf-core` / `tf-store`（本番依存）。`tf-evidence` / `tf-parsers` / `tf-engines` / `tf-findings` へは依存しない（Exporter は強型 CaseData を入力とし、Evidence snapshot・Parser・Engine の内部状態へ影響しないため）。CLI が `tf-export` へ CaseData を渡す設計。
- `tf-cli` の依存方向: `tf-cli` → 全 tf-* crate（最終統合 point）。main.rs は stdout/stderr を分離し、規範 §19.1 へ従う。
- `tf-engines` の `loader.rs` へ Phase 7 で `RuleLoadOptions::to_discovery_options` public method を追加した（CLI rules command が同じ設定を使えるようにするため）。
- Phase 8 品質保証とリリース（T8-001〜T8-027）は新規依存・新コマンド・新 module を追加しない。代わりに `tf-cli` へ Phase 8 の統合テスト 3 file を追加した: `crates/cli/tests/phase8_determinism_tests.rs`（T8-001〜T8-004・規範 §13.3・§21-7: threads 1/2/自動で canonical JSON byte 一致・run metadata 分離・hash map iteration 順非依存・regression baseline・6 test）・`crates/cli/tests/phase8_safety_tests.rs`（T8-010・T8-013〜T8-015・製品 §4.5・§13.2: 破損 fixture 群で panic 非発生・resource limit 到達（`tf_evidence::discover` で max_files truncated=true）・過大 header size・zero fill・入力 directory 内出力拒否・既存 file 上書き拒否・symlink skip・9 test）・`crates/cli/tests/phase8_compat_tests.rs`（T8-020〜T8-022・互換 §12・Schema §9・§21-15: compatibility acceptance 8項目エンドツーエンド確認・thread consistency・Timesketch 出力形式検証・全 JSONL 行の schema_version 検証・Case JSON 構造検証・6 test）。Phase 8 で追加したテストは合計 21 test。
- T8-012（解析中の入力変更を再現する integrity test）は `tf-evidence` の T2-018（snapshot 中書換 test で Event 非生成・規範 §21-3）でカバーする。T8-015（path traversal）は T2-020・T2-021（入出力分離）と T2-001（source_locator 正規化）でカバーし、Phase 8 の `phase8_safety_tests.rs` でエンドツーエンド検証（入力 directory 内出力拒否・既存 file 上書き拒否・symlink skip）を追加した。
- fuzz corpus（T8-011・F-025）を `fuzz/corpus/<target>/` 配下の 12 target へ格納した（各 target へ正常最小入力 + 破損/境界値入力の seed corpus・合計 34 file）。`fuzz/corpus/{core,lnk,prefetch,usn,evtx,registry,amcache,jump_lists,rule_loader,sigma,yara_x,correlation}/`。
- リリース記録（T8-023〜T8-027）を `docs/release/v1.0/` 配下へ作成した: `release_gate_checklist.md`（T8-027・roadmap §8 + 規範 §21 15項目 + 互換 §12 8項目の合否状況）・`fuzz_campaign_report.md`（T8-011・12 fuzz target + corpus + panic 境界設計）・`benchmark_report.md`（T8-023・smoke benchmark 実測値と測定条件）・`dependency_license_record.md`（T8-025・直接依存・許可ライセンス・advisory 例外 2件）・`external_specification_revisions.md`（T8-026・[MS-SHLLINK] v10.0・yara-x v1.19・TF-SIGMA-1.0・TF-TIMESKETCH-1.0・TF-ATTACK-1.0 revision 記録）・`timesketch_import_verification.md`（T8-021・TF-TIMESKETCH-1.0 形式検証）・`compatibility_acceptance_summary.md`（T8-020・互換 §12 全8項目の最終確認）。
- README.md（T8-024・製品 §13.2）をルートへ作成した。例は実際の合成 LNK fixture から生成し、`docs/examples/` へ格納した（`generate_examples.ps1` で自動生成・JSONL・JSON・Text・CSV・Timesketch・inspect・version 各形式）。手書きの例は使わない。
- Phase 8 で M6（v1.0 Stable）へ到達した。release gate checklist（roadmap §8）の全項目が合格。
