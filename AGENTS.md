...
- `tf-parsers` の統合テストは `crates/parsers/tests/` 配下（`framework_tests.rs`・`lnk_tests.rs`・`acceptance_tests.rs`）。規範 §9（Parser 契約）・互換 §4.4（LNK）・互換 §12（acceptance 8条件）・M2 縦割りを検証する。共通ヘルパー（合成 LNK fixture 生成・snapshot・ArtifactInstance 構築）は `tests/common/mod.rs`。
