// Registry Parser fuzz target（T4-092、F-025、互換 §12-2）。
//
// libFuzzer が生成した raw bytes を registry hive 形式と見なして Parser へ投げ、
...
use tf_parsers::registry::{PARSER_ID, PARSER_VERSION, RegistryParser};
...
fn make_context(data_len: u64) -> ParseContext {
    ...
    let evidence = EvidenceItem {
        evidence_id: id::evidence_id("SYSTEM", data_len, &sha),
        source_locator: "SYSTEM".to_string(),
        size: data_len,
        sha256: sha,
    ...
    let artifact_id = id::artifact_id(
        ...
        ArtifactSource::Registry.as_str(),
        PARSER_ID,
        PARSER_VERSION,
    );
