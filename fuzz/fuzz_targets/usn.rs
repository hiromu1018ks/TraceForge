// USN Journal Parser fuzz target（T4-092、F-025、互換 §12-2）。
//
// libFuzzer が生成した raw bytes を USN $J 形式と見なして Parser へ投げ、
...
use tf_parsers::usn::{PARSER_ID, PARSER_VERSION, UsnParser};
...
fn make_context(data_len: u64) -> ParseContext {
    ...
    let evidence = EvidenceItem {
        evidence_id: id::evidence_id("$UsnJrnl$J", data_len, &sha),
    ...
    let artifact_id = id::artifact_id(
        ...
        ArtifactSource::UsnJournal.as_str(),
        PARSER_ID,
        PARSER_VERSION,
    );
